import { getCurrentUserNav, getCurrentUserNav2 } from '@/api/user'
...
export const generator2 = (routeMap) => {
  return routeMap.map((item) => {
    ...
    if (item.edges && item.edges.submenus && item.edges.submenus.length > 0) {
      currentRoute.children = generator2(item.edges.submenus)
    }
    return currentRoute
  })
}
